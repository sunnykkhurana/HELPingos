...
require("class.phpmailer.php");
			...
			if(isset($_REQUEST['submit']))
				...
				$sql = "SELECT email, password FROM helpingos_member WHERE email='$to'";
				 $getpasswrd = mysql_query($sql);
				
				while($passfetch=mysql_fetch_assoc($getpasswrd)) {
					
					echo $encrypted= $passfetch['password'];
...
$message_body = 'Your Password is  "<strong>'.$decrypted.'</strong>". <br />Please inform us if you had not asked for your helpingos password <a href="#">Contact Us</a><br />';
  ...
  if ( $Mail->IsError() ) { // ADDED - This error checking was missing
	...
	header('location:account.php?inform=<h3>We have send your password to your mail ID please check and login here!</h3>');
    return TRUE;
  }
