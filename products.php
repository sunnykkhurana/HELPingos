				...
				<!--<div class="mens-toolbar">
					...
					<ul>
						<?php
							$query= "SELECT * FROM helpingos_product_category";
							$retval=mysql_query($query,$conn);
								$query= "SELECT * FROM helpingos_product WHERE pro_category_id='".$_REQUEST['cat_id']."'";
								$retval1=mysql_query($query,$conn);
								$productCount = mysql_num_rows($retval1); // count the output amount
								if ($productCount > 0) {
									while($product=mysql_fetch_assoc($retval1)){
								echo "<li>";
							...
							?>
								<!--<form id="cartform" name="cartform" method="post" action="add_to_cart.php">
									...
									<input type="submit" class="cbp-vm-icon cbp-vm-add" name="submit" id="button" value="Add to Shopping Cart" />
								</form>-->
							<?php
								echo    "</li>";
									}
									...
									echo "Sorry, there are no products in these category at present";	
							?>
					</ul>
				</div>
